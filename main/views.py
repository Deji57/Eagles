...
from .forms import * #ContactForm, CustomerForm, ProfileUpdateForm
from .models import * #AppInfo, Product, Category, Size, Customer, Cart, Order
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if customer.is_valid():
            ...
            newuser.pix = pix
            newuser.save()
            messages.success(request,f'dear {user} your account is creatded successfully')
            return redirect('homepage')
        else:
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if passupdate:
            ...
            messages.success(request, f'dear your password is updated successfully')
            return redirect('profile')
        else:
...
def payment(request):
    if request.method == 'POST':
        ...
        api_key = 'sk_test_80121ffba4b2e7391ca514aeda252a02a14f5524' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url
        cburl = 'http://16.170.226.49/thankyou' #thank you page #cb is call back
        ref = str(uuid.uuid4()) #reference id required by paystack as an additional refernce number
        order_no = profile.id
        ...
        data = {'reference':ref, 'amount':int(amount), 'email':email, 'callback_url':cburl, 'order_numer': order_no, 'currency':'NGN'}
